...
def display_state(
    ...
    """Display the current session state for Sale Deed drafting."""
    try:
        session = session_service.get_session(
            app_name=app_name, user_id=user_id, session_id=session_id
        )
        ...
        # Selected clauses
        clauses = session.state.get("clauses", [])
        if clauses:
            print("📑 Selected Clauses:")
            for clause in clauses:
                print(f"  - {clause}")
        else:
            print("📑 Selected Clauses: None")

        # Retrieved clauses
        retrieved = session.state.get("retrieved_clauses", [])
        if retrieved:
            print("🗃️ Retrieved Clauses:")
            for rc in retrieved:
                clause_type = rc.get("type", "Unknown")
                snippet = rc.get("text", "")[:60].replace("\n", " ") + "..."
                print(f"  - {clause_type}: \"{snippet}\"")
        else:
            print("🗃️ Retrieved Clauses: None")

        # Draft
        draft = session.state.get("draft")
        if draft:
            snippet = draft[:80].replace("\n", " ") + "..."
            print(f"✍️ Draft (preview): \"{snippet}\"")
        else:
            print("✍️ Draft: None")
        ...
        if interaction_history:
            ...
            for idx, entry in enumerate(interaction_history, 1):
                if isinstance(entry, dict):
                    action = entry.get("action", "unknown")
                    content = entry.get("content") or entry.get("selection") or entry
                    # Simplify display
                    summary = str(content)
                else:
                    summary = str(entry)
                snippet = summary[:60].replace("\n", " ") + "..."
                print(f"  {idx}. {snippet}")
        else:
